...
<head>
    ...
    <title>Rekapitulasi - Dashboard</title>
                        ...
                        <table id="myTable" class="display" style="width:100%;">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($rekap as $item) { ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= !is_null($item->tanggal_pemasukan) ? date('d F Y', strtotime($item->tanggal_pemasukan)) : '' ?> <?= !is_null($item->tanggal_pengeluaran) ? date('d F Y', strtotime($item->tanggal_pengeluaran)) : '' ?></td>
                                        <td><?= !is_null($item->desc_pemasukan) ? $item->desc_pemasukan : '' ?> <?= !is_null($item->desc_pengeluaran) ? $item->desc_pengeluaran : '' ?></td>
                                        <td><?= !is_null($item->jumlah_pemasukan) ? number_format($item->jumlah_pemasukan, 2) : '' ?> <?= !is_null($item->jumlah_pengeluaran) ? number_format($item->jumlah_pengeluaran, 2) : '' ?></td>
                                        <?php if ($item->type == 'pemasukan') { ?>
                                            <td class="flex justify-center" style="gap: 1rem;">
                                                <svg width="25px" height="25px" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M9.16421 9.66421L15.4142 3.41421L12.5858 0.585785L6.33579 6.83578L3.5 4L2 5.5V14H10.5L12 12.5L9.16421 9.66421Z" fill="#0356fc" />
                                                </svg>
                                                Pemasukan
                                            </td>
                                        <?php } else { ?>
                                            <td class="flex justify-center" style="gap: 1rem;">
                                                <svg width="25px" height="25px" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M14 2H5.50003L4.00003 3.5L6.83581 6.33579L0.585815 12.5858L3.41424 15.4142L9.66424 9.16421L12.5 12L14 10.5L14 2Z" fill="#fc1c03" />
                                                </svg>
                                                Pengeluaran
                                            </td>
                                        <?php } ?>
